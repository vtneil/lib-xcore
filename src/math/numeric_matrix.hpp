#ifndef LIB_XCORE_LINALG_NUMERIC_MATRIX_HPP
#define LIB_XCORE_LINALG_NUMERIC_MATRIX_HPP
...
namespace impl {
   ...
   * behavior or errors. This avoids heap usages for safety-critical and
  ...
  public:
    ...
    /**
     * Array constructor, construct from an array
     *
     * @param array Array of entries
     */
    constexpr explicit numeric_matrix_static_t(const T (&array)[Row][Col])
        : vector_(LIB_XCORE_NAMESPACE::detail::make_nested(array)) {}

    /**
     * Array of vectors as row constructor, construct from an array
     *
    ...
    FORCE_INLINE numeric_vector_static_t<T, Col> &operator[](const size_t index) { return vector_[index]; }

    FORCE_INLINE constexpr const numeric_vector_static_t<T, Col> &
    operator[](const size_t index) const { return vector_[index]; }

    FORCE_INLINE T &at(const size_t r_index, const size_t c_index) { return vector_[r_index][c_index]; };

    FORCE_INLINE constexpr const T &at(const size_t r_index, const size_t c_index) const { return vector_[r_index][c_index]; };

    FORCE_INLINE T &operator()(const size_t r_index, const size_t c_index) { return at(r_index, c_index); }

    FORCE_INLINE constexpr const T &operator()(const size_t r_index, size_t const c_index) const {
    ...
    numeric_matrix_static_t<T, Row, ORow> matmul_T(const numeric_matrix_static_t<T, ORow, OCol> &other) const {
      ...
      return mm_naive_A_BT(C, *this, other);
    }

    template<size_t ORow, size_t OCol>
    numeric_matrix_static_t<T, Col, OCol> T_matmul(const numeric_matrix_static_t<T, ORow, OCol> &other) const {
      numeric_matrix_static_t<T, Col, OCol> C;
      return mm_naive_AT_B(C, *this, other);
    }
    ...
    numeric_matrix_static_t<T, Order, Order> &operator^=(const size_t n) {
    ...
    numeric_matrix_static_t<T, Order, Order> operator^(size_t n) {
      ...
      numeric_matrix_static_t<T, Order, Order> product(move(identity()));
      while (n > 0) {
    ...
    /**
     * Calculate the nth power of this matrix in O(log n * mulO). Must be a square matrix.\n
     * If this matrix is not square, the compile-time error is thrown.
     *
     * @param n
     * @return A^n
     */
    numeric_matrix_static_t<T, Order, Order> matpow(const size_t n) {
      static_assert(static_is_a_square_matrix(), "Non-square matrix can\'t use power operator.");
      return operator^(n);
    }

    /**
     * Calculate the nth power of this matrix in O(n * mulO) naively. Must be a square matrix.\n
     * If this matrix is not square, the compile-time error is thrown.
     *
     * @param n
     * @return A^n
     */
    numeric_matrix_static_t<T, Order, Order> matpow_naive(const size_t n) {
      static_assert(static_is_a_square_matrix(), "Non-square matrix can\'t use power operator.");
      numeric_matrix_static_t<T, Order, Order> product = move(id());
      for (size_t i = 0; i < n; ++i) product *= *this;
      return product;
    }

    /**
     * Insert the other matrix into the current matrix, returning reference to the current matrix.\n
     ...
     * @tparam ORow Another matrix's row size
     * @tparam OCol Another matrix's column size
     ...
     * @tparam ORow Another matrix's row size
     * @tparam OCol Another matrix's column size
     * @param array Another matrix as 2D array
     * @return Reference to this matrix
     */
    template<size_t pos_row = 0, size_t pos_col = 0, size_t ORow, size_t OCol>
     ...
     * @return A slice of the current matrix
     */
     ...
     * @return Row at index as a vector
     */
    constexpr numeric_vector_static_t<T, Col> row(const size_t r_index) const {
     ...
     */
    numeric_vector_static_t<T, Row> col(const size_t c_index) const {
  ...
  public:
     ...
     * @tparam ORow
     * @tparam OCol
     * @param array Another matrix as an array
     * @return
     */
    template<size_t ORow, size_t OCol>
     ...
     * @tparam ORow
     * @tparam OCol
     * @param array Another matrix as array
     * @return
     */
    template<size_t ORow, size_t OCol>
    ...
    /**
     * Checks equality of this matrix and the other matrix with a float/double threshold using
     ...
     */
    template<size_t ORow, size_t OCol>
    bool float_equals(const T (&array)[ORow][OCol], real_t threshold = XCORE_FLOAT_THRESHOLD) const {
      if (Row != ORow || Col != OCol) return false;
      for (size_t i = 0; i < Row; ++i)
        if (!vector_[i].float_equals(array[i], threshold)) return false;
      return true;
    }
     ...
     * @param other
     */
    void swap(numeric_matrix_static_t &other) noexcept {
  ...
  private:
    ...
    void allocate_zero() {
      vector_ = move(numeric_vector_static_t<numeric_vector_static_t<T, Col>, Row>());
    }

    void allocate_fill(T fill) {
      vector_ = move(numeric_vector_static_t<numeric_vector_static_t<T, Col>, Row>(numeric_vector_static_t<T, Col>(fill)));
    }
    ...
    void steal(numeric_matrix_static_t &&other) { vector_ = move(other.vector_); }

    template<size_t ORow, size_t OCol, size_t M, size_t N>
    void helper_insert_major(const size_t pos_row, const numeric_matrix_static_t<T, ORow, OCol> (&blocks)[M][N]) {
      if (pos_row < M) {
    ...
    template<size_t ORow, size_t OCol, size_t M, size_t N>
    void helper_insert_minor(const size_t pos_row, const size_t pos_col,
    ...
    template<size_t ORow, size_t OCol>
    void helper_insert_unsafe(const size_t pos_row, const size_t pos_col, const numeric_matrix_static_t<T, ORow, OCol> &M) {
      for (size_t i = 0; i < ORow; ++i)
        for (size_t j = 0; j < OCol; ++j)
          vector_[pos_row + i][pos_col + j] = M[i][j];
    }
    ...
    static constexpr bool is_multiple_of_2(const size_t x) { return x != 0 && !(x & (x - 1)); }
    ...
    template<size_t ORow, size_t X, size_t OCol>
    static numeric_matrix_static_t<T, ORow, OCol> &mm_naive_A_BT(numeric_matrix_static_t<T, ORow, OCol>    &C,
                                                                 const numeric_matrix_static_t<T, ORow, X> &A,
    ...
    template<size_t ORow, size_t X, size_t OCol>
    static numeric_matrix_static_t<T, ORow, OCol> &mm_naive_AT_B(numeric_matrix_static_t<T, ORow, OCol>    &C,
                                                                 const numeric_matrix_static_t<T, X, ORow> &A,
                                                                 const numeric_matrix_static_t<T, X, OCol> &B) {
      for (size_t i = 0; i < ORow; ++i) {
        numeric_vector_static_t<T, OCol> &row_C = C[i];
        for (size_t k = 0; k < X; ++k) {
    ...
    static numeric_matrix_static_t<T, OSize, OSize> inv_ut(numeric_matrix_static_t<T, OSize, OSize> &U) {
      static_assert(static_is_a_square_matrix(), "Can only inverse a square matrix.");
      numeric_matrix_static_t<T, OSize, OSize> tmp = move(U.transpose());
  ...
  public:
    ...
    /**
     * Returns an identity matrix of the current dimension.\n
     * If this matrix is not square, the compile-time error is thrown.
     *
    ...
    static constexpr numeric_matrix_static_t diagonals(const T &value) {
      return numeric_matrix_static_t(LIB_XCORE_NAMESPACE::detail::make_diagonal(numeric_vector_static_t<T, Order>(value).arr_));
    }
    ...
    static constexpr numeric_matrix_static_t diagonals(const T (&array)[Order]) {
      return numeric_matrix_static_t(LIB_XCORE_NAMESPACE::detail::make_diagonal(array));
    }
 ...
 */
template<typename T, size_t Row, size_t Col>
T det(const LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col> &A) { return A.det(); }
 ...
 */
template<typename T, size_t Row, size_t Col>
T tr(const LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col> &A) { return A.tr(); }
 ...
 */
template<typename T, size_t Row, size_t Col>
LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col> inv(const LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col> &A) {
  return A.inv();
}
 ...
 */
template<typename T, size_t Row, size_t Col>
LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col> RRE(const LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col> &A) {
  return A.RRE();
}

namespace impl {
  /**
   * Wrapper class for LU-decomposed matrix comprising L and U square matrices.
   *
   * @tparam T
   * @tparam OSize
   */
  ...
  private:
    ...
    using Pair_t   = pair<numeric_matrix_static_t<T, OSize, OSize>, numeric_matrix_static_t<T, OSize, OSize>>;
  ...
  public:
    ...
    numeric_matrix_static_lu_t(numeric_matrix_static_lu_t &&other) noexcept : l_(move(other.l_)),
                                                                              u_(move(other.u_)) {}
...
using generic_matrix = LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<T, Row, Col>;
 ...
 * performance-critical systems.
 * \n
 * This numeric matrix uses a real type (real_t), defined as double).
 *
 * @tparam Row Row dimension
 * @tparam Col Column dimension
 */
template<size_t Row, size_t Col = Row>
using numeric_matrix = LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_t<real_t, Row, Col>;

template<size_t OSize>
using numeric_matrix_lu = LIB_XCORE_NAMESPACE::impl::numeric_matrix_static_lu_t<real_t, OSize>;

/**
 * Creates an empty (zeroes-filled) numeric matrix.
 *
 * @tparam Row Row dimension
 * @tparam Col Column dimension
...
/**
 * Creates a diagonal matrix filled with an array of values.
 *
 * @tparam Order
...
#endif  //LIB_XCORE_LINALG_NUMERIC_MATRIX_HPP
