...
public:
  ...
  kalman_filter_t &update(const numeric_vector<M_> &z) {
    ...
    numeric_matrix<N_, M_> P_H_t = move(P_.matmul_T(H_));
    numeric_matrix<M_, M_> S_    = move(H_ * P_H_t + R_);
    numeric_matrix<N_, M_> K_    = move(P_H_t * S_.inverse());

    x_ += K_ * y_;
    P_ = move((numeric_matrix<N_, N_>::identity() - K_ * H_) * P_);
