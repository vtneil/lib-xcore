...
namespace impl {
  ...
  private:
    template<size_t... I>
    constexpr numeric_vector_static_t(const T &fill, index_sequence<I...>)
  ...
  private:
    template<size_t... I>
    constexpr numeric_vector_static_t(const T (&array)[Size], index_sequence<I...>)
        : arr_{array[I]...} {}
  ...
  public:
    ...
    FORCE_INLINE T &operator[](const size_t index) { return *(arr_ + index); }

    FORCE_INLINE constexpr const T &operator[](const size_t index) const { return *(arr_ + index); }

    FORCE_INLINE T &at(const size_t index) { return operator[](index); };

    FORCE_INLINE constexpr const T &at(const size_t index) const { return operator[](index); };

    FORCE_INLINE T &operator()(const size_t index) { return at(index); }

    FORCE_INLINE constexpr const T &operator()(const size_t index) const { return at(index); }
    ...
    numeric_vector_static_t &operator=(numeric_vector_static_t &&other) noexcept {
      if (this != &other)
        for (size_t i = 0; i < Size; ++i) arr_[i] = move(other.arr_[i]);
      return *this;
    }
     ...
     * Calculates inner product of this vector (LHS transposed) and the other vector of the same dimension (RHS).
     *
     * @param array Another vector as an array
     * @return Inner product
     */
     ...
     *
     * @tparam OSize Another vector's dimension
     * @param other Another vector
     ...
     *
     * @tparam OSize Another array's dimension
     ...
     */
    template<size_t OSize>
    bool float_equals(const T (&array)[OSize], real_t threshold = XCORE_FLOAT_THRESHOLD) const {
      if (Size != OSize) return false;
      for (size_t i = 0; i < Size; ++i)
        if (abs(arr_[i] - array[i]) > threshold) return false;
      return true;
    }
    ...
    /**
     * Converts this vector to a matrix representation of a row vector
     *
     ...
     * @return Vector's dimension (size)
     */
    [[nodiscard]] constexpr size_t dim() const { return Size; }
     ...
     * @return Vector's dimension (size)
     */
    [[nodiscard]] constexpr size_t dimension() const { return Size; }
     ...
     * @param other Another vector
     */
    void swap(numeric_vector_static_t &other) noexcept {
      for (size_t i = 0; i < Size; ++i) LIB_XCORE_NAMESPACE::swap(arr_[i], other.arr_[i]);
    }
    ...
    numeric_vector_static_t &assign_to(Ts &...vars) {
      ...
      LIB_XCORE_NAMESPACE::detail::helper_assign_to<T, Size, sizeof...(Ts), 0>(arr_, vars...);
      return *this;
    }
    ...
    numeric_vector_static_t &assign_from(const Ts &...vars) {
      static_assert(sizeof...(Ts) <= Size,
                    "Number of variables must not be greater than vector\'s dimension.");
      LIB_XCORE_NAMESPACE::detail::helper_assign_from<T, Size, sizeof...(Ts), 0>(arr_, vars...);
      return *this;
    }

    FORCE_INLINE LIB_XCORE_NAMESPACE::detail::output_assignment_chain<T, Size, 1> operator>>(T &var) const {
      var = arr_[0];
      return LIB_XCORE_NAMESPACE::detail::output_assignment_chain<T, Size, 1>(arr_);
    }

    FORCE_INLINE LIB_XCORE_NAMESPACE::detail::input_assignment_chain<T, Size, 1> operator<<(const T &var) {
      arr_[0] = var;
      return LIB_XCORE_NAMESPACE::detail::input_assignment_chain<T, Size, 1>(arr_);
    }

  private:
    void allocate_zero() { allocate_fill(T()); }

    void allocate_fill(const T &fill) { LIB_XCORE_NAMESPACE::fill(arr_, arr_ + Size, fill); }

    void allocate_from(const numeric_vector_static_t &other) {
      static_cast<void>(LIB_XCORE_NAMESPACE::copy(other.arr_, other.arr_ + Size, arr_));
    }

    void allocate_from(const T (&array)[Size]) {
      static_cast<void>(LIB_XCORE_NAMESPACE::copy(array, array + Size, arr_));
    }
...
namespace detail {
  ...
  struct size_sum<Class<T, Size>, Class<T, Sizes>...> {
    static constexpr size_t value = Size + size_sum<LIB_XCORE_NAMESPACE::impl::numeric_vector_static_t<T, Sizes>...>::value;
  };
}  // namespace detail

template<typename T, size_t N>
using generic_vector = LIB_XCORE_NAMESPACE::impl::numeric_vector_static_t<T, N>;
...
constexpr generic_vector<T, LIB_XCORE_NAMESPACE::detail::size_sum<generic_vector<T, S1>, generic_vector<T, S2>, generic_vector<T, Ss>...>::value>
 ...
 * performance-critical systems.
 * \n
 * This numeric vector uses a real type (real_t), defined as double).
 *
 * @tparam Size Vector dimension
 */
template<size_t Size>
using numeric_vector = LIB_XCORE_NAMESPACE::impl::numeric_vector_static_t<real_t, Size>;
...
constexpr numeric_vector<LIB_XCORE_NAMESPACE::detail::size_sum<numeric_vector<S1>, numeric_vector<S2>, numeric_vector<Ss>...>::value>
