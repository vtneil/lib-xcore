...
int main(int, char *[]) {
  ...
  numeric_matrix<3, 4> A({
    {1, 2, 3, 4},
    {4, 5, 6, 1},
    {7, 8, 9, 0}
  });
  numeric_matrix<4, 2> B({
    {3,  4},
    {1, -1},
    {6,  1},
    {4,  5}
  ...
  auto                 v1 = make_numeric_vector({1, 2, 3, 4});
  auto                 M2 = make_quad_matrix(A, A, A, A);
  ...
  for (size_t i = 0; i < N; ++i) {
    for (size_t j = 0; j < N; ++j) {
      X[i][j] = static_cast<real_t>(i * N + j) / 762.;
      Y[i][j] = static_cast<real_t>(i * N + j) / 1983.;
    }
  }
  ...
  assert(numeric_vector<3>({-2, 2, 1}).normalize().float_equals({-2. / 3., 2. / 3., 1. / 3.}));
  assert(A.equals({
    {1, 2, 3, 4},
    {4, 5, 6, 1},
    {7, 8, 9, 0}
  ...
  assert((numeric_matrix<A.r(), A.c()>(A).equals({
    {1, 2, 3, 4},
    {4, 5, 6, 1},
    {7, 8, 9, 0}
  })));
  ...
  assert(!A.is_square());
  assert(C.is_square());
  assert((C ^ 9).equals(512. * numeric_matrix<3>({
                                 {1160, 2561, 2070},
                                 {2561, 5791, 4631},
                                 {2070, 4631, 3721}
  })));
  ...
  assert(det(C) == -8.);
  assert(tr(C) == 8.);
  assert(inv(C) == 0.5 * numeric_matrix<3>({
                           {-1, -1,  2},
                           {-1,  0,  1},
                           { 2,  1, -2}
  }));
  ...
  assert((Diag1 == make_numeric_matrix({
                     {1, 0, 0},
                     {0, 2, 0},
                     {0, 0, 3}
  })));
  assert((Diag2 == make_numeric_matrix({
                     {5, 0, 0},
                     {0, 5, 0},
                     {0, 0, 5}
  })));
